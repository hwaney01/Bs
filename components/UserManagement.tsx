    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
            <div className="bg-slate-800 p-8 rounded-lg shadow-2xl w-full max-w-md relative">
                ...
                <h2 className="text-2xl font-bold mb-6 text-white">{user ? 'Edit User' : 'Add New User'}</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label htmlFor="password" className="block text-sm font-medium text-slate-300 mb-1">Password {user ? '(leave blank to keep current)' : ''}</label>
                    ...
                    <div className="flex justify-end pt-4">
                        ...
                        <button type="submit" className="px-4 py-2 bg-indigo-600 text-white font-semibold rounded-md hover:bg-indigo-700 transition-colors">Save</button>
                    </div>
                </form>
            </div>
        </div>
                                ...
                                <tr key={user.id} className="hover:bg-slate-700/30">
                                    ...
                                    <td className="p-4">
                                        <div className="flex justify-center items-center space-x-2">
                                            <button onClick={() => handleOpenModal(user)} className="p-2 text-slate-400 hover:text-yellow-400 transition-colors" title="Edit User"><EditIcon /></button>
                                            <button onClick={() => handleDeleteUser(user.id, user.username)} className="p-2 text-slate-400 hover:text-red-400 transition-colors" title="Delete User"><DeleteIcon /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
